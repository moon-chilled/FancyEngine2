...
cl_object eval(string s) {
	return s.lstr.cl_eval;
}
void init_ecl() {
	cl_boot(1, cast(char**)[cast(char*)[0].ptr].ptr);
	info("initiated ECL");
	eval("(+ 1 1)"); // sanity check
}
void shutdown_ecl() {
	cl_shutdown();
	info("shutdown ECL");
...
__gshared extern (C) nothrow @nogc {
	...
	enum Cnil = cast(cl_object)cl_type.t_list;
	enum cl_type {
		t_start = 0,
		t_list = 1,
		t_character = 2,
...
extern(C) {
    ...
    void ecl_def_c_function_va(cl_object, cl_lispunion* function(c_long, ...)) @nogc nothrow;
    ...
    c_long ecl_to_fixnum(cl_object) @nogc nothrow;
    ...
    c_long fixint(cl_object) @nogc nothrow;
    ...
    cl_object ecl_make_double_float(double) @nogc nothrow;
    ...
    float ecl_to_float(cl_object) @nogc nothrow;
    double ecl_to_double(cl_object) @nogc nothrow;
    ...
    static cl_type ecl_t_of(cl_object) @nogc nothrow;
