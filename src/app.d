...
int real_main(string[] args) {
	...
	faux.expose_fun("play_sound", (ScriptVar s) => s.match!(
		(ISound s) => audio.play(s),
		_ => error("Tried to play non-sound object '%s'", s)));
	faux.expose_fun("pause_sound", (ScriptVar s) => s.match!(
		(ISound s) => audio.pause(s),
		_ => error("Tried to pause non-sound object '%s'", s)));
	faux.expose_fun("rewind_sound", (ScriptVar s) => s.match!(
		(ISound s) => audio.rewind(s),
		_ => error("Tried to rewind non-sound object '%s'", s)));
	faux.expose_fun("stop_sound", (ScriptVar s) => s.match!(
		(ISound s) => audio.stop(s),
		_ => error("Tried to stop non-sound object '%s'", s)));
	faux.expose_fun("kill_sound", (ScriptVar s) => s.match!(
		(ISound s) => audio.kill(s),
		_ => error("Tried to kill non-sound object '%s'", s)));
